...
- Show HN draft (ángulo seguridad, ver §0b): título candidato "Show HN: Builders in a Box — a personal AI agent box that isn't one of the 42,665 exposed ones". Primer párrafo = la historia de Paco (el regalo) + el dato 93,4% auth bypass. Nombrar Cowork con honestidad ("if you just want Claude in the background, Cowork does that; this is for when the agent needs YOUR machine"). Sin jerga de módulos.
